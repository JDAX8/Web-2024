...
export const BigChannel = () => {
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 3) % stream_cards.length);
  };

  const handlePrev = () => {
    setCurrentIndex(
      (prevIndex) => (prevIndex - 3 + stream_cards.length) % stream_cards.length
    );
  };
  return (
    <div className="b-live-channels">
      {stream_cards.slice(currentIndex, currentIndex + 3).map((stream) => (
